jQuery(document).ready(function() {
	...
	$('.features .point').hover(function() {
		$(this).css({'opacity': '1'});
		var this_par = $(this).parent();
		$('.special_text', this_par).show(300);
		$('.feat_pos_img', this_par).show(300);
	}, function() {
		$(this).css({'opacity': '0.5'});
		var this_par = $(this).parent();
		$('.special_text', this_par).hide(300);
		$('.feat_pos_img', this_par).hide(300);
	})

	$('.effect li img').hover(function() {
		var this_class = $(this).attr('class'),
			this_par = $(this).parent();
		$('.angle', this_par).fadeIn(0);
		$('.effect_wrap .'+this_class).fadeIn(100).css({'display': 'inline-block'});
	}, function() {
		var this_class = $(this).attr('class'),
			this_par = $(this).parent();
		$('.angle', this_par).fadeOut(0);
		$('.effect_wrap .'+this_class).fadeOut(100);
	});

	$('.header .nav a[href^="#"]').click(function(){
		var el = $(this).attr('href');
		$('body').animate({
			scrollTop: $(el).offset().top - 94}, {queue:false, duration:1000});
		return false; 
	});
